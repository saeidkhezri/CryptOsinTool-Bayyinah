...
class SecureStorageManager(context: Context) {
    ...
    private fun getSecretKey(): SecretKey {
        return try {
            ...
            (keyStore.getEntry(KEY_ALIAS, null) as? KeyStore.SecretKeyEntry)?.secretKey
                ?: fallbackKey ?: createEphemeralFallbackKey().also { fallbackKey = it }
        } catch (_: Throwable) {
            if (fallbackKey == null) {
                val randomBytes = ByteArray(32)
                SecureRandom().nextBytes(randomBytes)
    ...
    private fun createEphemeralFallbackKey(): SecretKey {
        val randomBytes = ByteArray(32)
        SecureRandom().nextBytes(randomBytes)
    ...
    fun encrypt(plainText: String): String {
        ...
        return try {
            ...
            throw IllegalStateException("Secure encryption is unavailable; refusing to store plaintext secret", e)
        }
    }
